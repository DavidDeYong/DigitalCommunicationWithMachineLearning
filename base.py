...
class ClasificadorBase(ABC):
    ...
    # Parámetros para medición robusta de inferencia
    _N_WARMUP   = 5    # llamadas de calentamiento (descartar)
    _N_MEDICION = 20   # llamadas promediadas para la medición final

    def predict(self, X: np.ndarray) -> np.ndarray:
        """
        Predice etiquetas para X. Mide el tiempo de inferencia unitaria
        promediando sobre múltiples llamadas con warmup previo para
        evitar artefactos de inicialización de PyTorch/sklearn.
        ...
        # Warmup: descartar primeras llamadas (overhead de compilación JIT,
        # caché de CPU, inicialización de CUDA, etc.)
        for _ in range(self._N_WARMUP):
            _ = self._predict_interno(muestra)

        # Medición promediada
        tiempos = []
        for _ in range(self._N_MEDICION):
            t0 = time.perf_counter()
            _ = self._predict_interno(muestra)
            tiempos.append(time.perf_counter() - t0)

        # Usar mediana (más robusta ante outliers que la media)
        self._tiempo_inferencia_1 = float(np.median(tiempos))

        # Predicción completa sobre el conjunto de test
